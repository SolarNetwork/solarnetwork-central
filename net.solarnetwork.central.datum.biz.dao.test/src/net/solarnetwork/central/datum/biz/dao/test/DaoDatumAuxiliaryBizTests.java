...
import net.solarnetwork.central.daum.biz.dao.DaoDatumAuxiliaryBiz;
...
public class DaoDatumAuxiliaryBizTests {
	...
	private GeneralNodeDatumAuxiliary testGenAux() {
		...
		genAux.setCreated(new DateTime().hourOfDay().roundFloorCopy());
	...
	private DatumAuxiliaryEntity testAuxEntity(DateTime date, UUID streamId) {
		GeneralNodeDatumSamples sf = new GeneralNodeDatumSamples();
		sf.putAccumulatingSampleValue("foo", 1);
		...
		GeneralDatumMetadata meta = new GeneralDatumMetadata();
		meta.putInfoValue("bim", "pow");
		return new DatumAuxiliaryEntity(streamId, JodaDateUtils.fromJodaToInstant(date),
				DatumAuxiliaryType.Reset, null, sf, ss, "Note.", meta);
	}
	...
	public void store() {
		...
		DatumAuxiliaryPK entId = new DatumAuxiliaryPK(meta.getStreamId(),
				JodaDateUtils.fromJodaToInstant(genAux.getCreated()), DatumAuxiliaryType.Reset);
	...
	public void get() {
		// GIVEN
		GeneralNodeDatumAuxiliaryPK id = new GeneralNodeDatumAuxiliaryPK(TEST_NODE_ID, new DateTime(),
				TEST_SOURCE_ID);
	...
	public void remove() {
		...
		// THEN
		asssertCriteria(metaCriteriaCaptor.getValue());
		assertThat("PK deleted", entCaptor.getValue().getId(),
				equalTo(new DatumAuxiliaryPK(meta.getStreamId(), fromJodaToInstant(id.getCreated()),
	...
	public void move_sameStream() {
		...
		GeneralNodeDatumAuxiliary toGenAux = new GeneralNodeDatumAuxiliary(
				new GeneralNodeDatumAuxiliaryPK(TEST_NODE_ID, genAux.getCreated().plusHours(1),
						TEST_SOURCE_ID),
		...
		DatumAuxiliaryPK fromEntId = new DatumAuxiliaryPK(meta.getStreamId(),
				JodaDateUtils.fromJodaToInstant(genAux.getCreated()), DatumAuxiliaryType.Reset);
		Capture<DatumAuxiliaryEntity> entCaptor = new Capture<>();
	...
	public void move_differentStream() {
		...
		GeneralNodeDatumAuxiliary genAux = testGenAux();
		GeneralNodeDatumAuxiliary toGenAux = new GeneralNodeDatumAuxiliary(
				new GeneralNodeDatumAuxiliaryPK(TEST_NODE_ID, genAux.getCreated().plusHours(1), "b"),
				genAux.getSamplesFinal(), genAux.getSamplesStart());
		...
		DatumAuxiliaryPK fromEntId = new DatumAuxiliaryPK(metaFrom.getStreamId(),
				JodaDateUtils.fromJodaToInstant(genAux.getCreated()), DatumAuxiliaryType.Reset);
		Capture<DatumAuxiliaryEntity> entCaptor = new Capture<>();
		expect(datumAuxiliaryDao.move(eq(fromEntId), capture(entCaptor))).andReturn(true);
	...
	public void find() {
		...
		DatumAuxiliaryEntity ent = testAuxEntity(new DateTime(), streamId);
		...
		criteria.setStartDate(new DateTime().hourOfDay().roundFloorCopy());
		criteria.setEndDate(new DateTime().hourOfDay().roundCeilingCopy());
		...
		assertThat("Datum ID returned from meta", s.getId(),
				equalTo(new GeneralNodeDatumAuxiliaryPK(TEST_NODE_ID,
						JodaDateUtils.toJoda(ent.getTimestamp(), meta.getTimeZoneId()), TEST_SOURCE_ID,
						DatumAuxiliaryType.Reset)));
