/* ==================================================================
 * IbatisBaseGenericDaoSupport.java - Dec 12, 2012 4:11:53 PM
 * 
 ...
 * iBATIS query names. The {@code queryForId}, {@code queryForAll},
 ...
 * 
 * <dt>queryForId</dt>
 * <dd>The name of the iBATIS SQL query to load an entity based on its primary
 ...
 * 
 * <dt>queryForAll</dt>
 * <dd>The name of the iBATIS SQL query to return a list of entities, supporting
 * a custom sort order. Defaults to <code>findall-<em>DomainClass</em></code>
 * where <em>DatumClass</em> is the simple name of the configured
 * {@code domainClass}.</dd>
 * 
 * <dt>insert</dt>
...
public abstract class IbatisBaseGenericDaoSupport<T extends Entity<PK>, PK extends Serializable> extends
	...
	protected PK handleInsert(T datum) {
		...
		try {
			return pkClass.cast(id);
		} catch ( ClassCastException e ) {
			log.debug("Can't cast primary key result {} from insert to expected type {}", id, pkClass);
		}
		return null;
	}
	...
	protected <E extends Identity<Long>> void handleRelation(Long parentId, E newObject,
		...
		Long oldObjectId = (Long) getSqlMapClientTemplate().queryForObject(findForParentQuery, parentId);
	...
	protected <E extends Identity<Long>> Long handleChildRelation(T parent, E child,
		...
		if ( child == null ) {
			...
			if ( parent.getId() != null ) {
				oldObjectId = (Long) getSqlMapClientTemplate().queryForObject(findForParentQuery,
						parent.getId());
			}
			if ( oldObjectId != null ) {
