 ...
 * 
 * <p>The scheduler works by sending OSGi EventAdmin events. Post a
 * {@link #TOPIC_JOB_REQUEST} event to schedule a job. The job will 
 * run at the time specified by the {@link #JOB_DATE} property, triggering
 * a new event using the topic specified by the request
 * {@link #JOB_TOPIC} property.</p>
 * 
 * <p>When the job executes, it will expect an acknowledgment event to
 * be posted, either {@link #TOPIC_JOB_COMPLETE} or {@link #TOPIC_JOB_FAILURE},
 * so the job can tell if the work was executed or not. The job will wait
 * at most {@link #JOB_MAX_WAIT} milliseconds (using a default value if
 * this is not specified on the job request), after which it will time out.</p>
 * 
 * @author matt
 * @version $Revision$
 */
