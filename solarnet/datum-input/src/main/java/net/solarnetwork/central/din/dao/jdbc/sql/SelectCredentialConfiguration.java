...
public class SelectCredentialConfiguration
	...
	private void sqlCore(StringBuilder buf) {
		buf.append("""
				SELECT dc.user_id, dc.id, dc.created, dc.modified, dc.enabled
					, dc.username, dc.password, dc.expires
				FROM solardin.din_credential dc
				""");
	}
	...
	private void sqlWhere(StringBuilder buf) {
		...
		if ( filter.hasUserCriteria() ) {
			idx += whereOptimizedArrayContains(filter.getUserIds(), "dc.user_id", where);
		}
		if ( filter.hasCredentialCriteria() ) {
			idx += whereOptimizedArrayContains(filter.getCredentialIds(), "dc.id", where);
		}
		if ( filter.hasEnabledCriteria() ) {
			where.append("\tAND dc.enabled = ?\n");
			idx += 1;
		}
		if ( idx > 0 ) {
	...
	private void sqlOrderBy(StringBuilder buf) {
		buf.append("ORDER BY dc.user_id, dc.id");
	}
