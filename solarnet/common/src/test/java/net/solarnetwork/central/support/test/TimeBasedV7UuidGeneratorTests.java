...
import static net.solarnetwork.central.support.UuidUtils.extractTimestampV7;
...
public class TimeBasedV7UuidGeneratorTests {
	...
	public void timeOrder() {
		...
		log.debug("Generated {} v7 UUIDs: [{}]", uuids.size(),
				uuids.stream().map(u -> "%s (%s)".formatted(u, extractTimestampV7(u, false)))
						.collect(joining("\n\t", "\n\t", "\n")));
	...
	public void timeOrder_withMicros() {
		...
		for ( int i = 0; i < count; i++ ) {
			...
			if ( r.nextBoolean() ) {
				try {
					Thread.sleep(Math.abs(r.nextLong(100)));
				} catch ( InterruptedException e ) {
					// ignore
				}
			}
		...
		log.debug("Generated {} v7 + micros UUIDs: [{}]", uuids.size(),
				uuids.stream().map(u -> "%s (%s)".formatted(u, extractTimestampV7(u, true)))
						.collect(joining("\n\t", "\n\t", "\n")));

		// THEN
		long prevTimestamp = 0;
		for ( UUID uuid : uuids ) {
			Instant curr = UuidUtils.extractTimestampV7(uuid, true);
			assertThat("UUID time is never decreasing", curr.toEpochMilli(),
					is(greaterThanOrEqualTo(prevTimestamp)));
			prevTimestamp = curr.toEpochMilli();
		}
