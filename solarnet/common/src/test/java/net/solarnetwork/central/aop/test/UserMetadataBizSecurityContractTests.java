/* ==================================================================
 * UserMetadataBizSecurityContractTests.java - 22/09/2026 2:19:25 pm
 *
...
/**
 * Verify the {@link UserMetadataBizSecurityContract} for
 * {@link UserMetadataBiz} with the {@code UserMetadataSecurityAspect} aspect
 * applied.
 *
 * @author matt
 * @version 1.0
...
public class UserMetadataBizSecurityContractTests {

	private final TestTenants tenants = new TestTenants();

	private SecuredProxy<UserMetadataBiz> proxy() {
		return securedProxy((UserMetadataBiz) mock(DaoUserMetadataBiz.class),
				new UserMetadataSecurityAspect(tenants.ownershipDao()));
	}

	@TestFactory
	public Stream<DynamicNode> securityContract() {
		return UserMetadataBizSecurityContract.contract(tenants).dynamicTests(this::proxy);
	}

	@Test
	public void pointcutsMatch() {
		// @formatter:off
		then(unmatchedPointcuts(UserMetadataSecurityAspect.class, DaoUserMetadataBiz.class))
			.as("Every UserMetadataSecurityAspect pointcut matches a UserMetadataBiz method")
			.isEmpty()
			;
		// @formatter:on
	}
