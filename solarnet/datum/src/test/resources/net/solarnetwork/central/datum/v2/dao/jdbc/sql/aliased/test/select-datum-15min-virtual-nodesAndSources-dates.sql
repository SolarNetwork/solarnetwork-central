...
, d AS (
	...
	INNER JOIN (
		...
		FROM s
		INNER JOIN solardatm.rollup_datm_for_time_span_slots(s.orig_stream_id, ?, ?, ?) datum ON datum.stream_id = s.orig_stream_id
		GROUP BY datum.stream_id, ts
		) AS ds ON ds.stream_id = s.orig_stream_id
)
