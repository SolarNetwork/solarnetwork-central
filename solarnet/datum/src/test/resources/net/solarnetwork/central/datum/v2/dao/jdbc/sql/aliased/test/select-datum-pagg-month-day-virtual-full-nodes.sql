WITH rs AS (
	SELECT s.stream_id
		...
		, s.names_i
		, s.names_a, COALESCE(l.time_zone, 'UTC') AS time_zone, s.orig_stream_id
...
, d AS (
	...
	INNER JOIN (
		...
		GROUP BY s.stream_id, date_trunc('month', datum.ts_start AT TIME ZONE s.time_zone) AT TIME ZONE s.time_zone
		HAVING COUNT(*) > 0
	...
	SELECT s.vstream_id AS stream_id,
		...
		datum.read_a
	FROM s
	INNER JOIN solardatm.agg_datm_monthly datum ON datum.stream_id = s.orig_stream_id
	WHERE datum.ts_start >= ? AT TIME ZONE s.time_zone
		AND datum.ts_start < ? AT TIME ZONE s.time_zone
