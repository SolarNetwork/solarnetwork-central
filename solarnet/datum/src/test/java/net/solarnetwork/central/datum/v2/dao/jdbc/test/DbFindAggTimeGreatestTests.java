 ...
 * @version 1.0
 */
public class DbFindAggTimeGreatestTests extends BaseDatumJdbcTestSupport {

	private List<AggregateDatum> findAggTimeGreatest(UUID[] streamIds, Aggregation kind) {
		return jdbcTemplate.execute(new ConnectionCallback<List<AggregateDatum>>() {
			...
			public List<AggregateDatum> doInConnection(Connection con)
				...
				try (CallableStatement stmt = con
						.prepareCall("{call solardatm.find_agg_time_greatest(?::uuid[],?)}")) {
	...
	@Test
	public void findAggTimeGreatest_noData() {
		// GIVEN
		UUID streamId = UUID.randomUUID();

		// WHEN
		for ( Aggregation kind : aggs() ) {
			List<AggregateDatum> results = findAggTimeGreatest(new UUID[] { streamId }, kind);
	...
	public void findAggTimeGreatest_hour_oneStream() throws IOException {
		...
		// WHEN
		List<AggregateDatum> results = findAggTimeGreatest(new UUID[] { streamId }, Aggregation.Hour);

		// THEN
		ZonedDateTime date = ZonedDateTime.of(2020, 6, 1, 21, 0, 0, 0, ZoneOffset.UTC);
		assertThat("Result for one stream", results, hasSize(1));
		assertAggregateDatum("Greatest result returned", results.get(0),
				new AggregateDatumEntity(streamId, date.toInstant(), Aggregation.Hour,
						propertiesOf(decimalArray("1.9", "9.1"), decimalArray("800"), null, null),
						statisticsOf(
	...
	public void findAggTimeGreatest_day_oneStream() throws IOException {
		...
		// WHEN
		List<AggregateDatum> results = findAggTimeGreatest(new UUID[] { streamId }, Aggregation.Day);

		// THEN
		ZonedDateTime date = ZonedDateTime.of(2020, 6, 30, 0, 0, 0, 0, ZoneOffset.UTC);
		assertThat("Result for one stream", results, hasSize(1));
		assertAggregateDatum("Greatest result returned", results.get(0),
				new AggregateDatumEntity(streamId, date.toInstant(), Aggregation.Day,
						propertiesOf(decimalArray("1.8", "8.1"), decimalArray("700"), null, null),
						statisticsOf(
	...
	public void findAggTimeGreatest_month_oneStream() throws IOException {
		...
		// WHEN
		List<AggregateDatum> results = findAggTimeGreatest(new UUID[] { streamId }, Aggregation.Month);

		// THEN
		ZonedDateTime date = ZonedDateTime.of(2020, 12, 1, 0, 0, 0, 0, ZoneOffset.UTC);
		assertThat("Result for one stream", results, hasSize(1));
		assertAggregateDatum("Greatest result returned", results.get(0),
	...
	public void findAggTimeGreatest_hour_twoStreams() throws IOException {
		...
		// WHEN
		List<AggregateDatum> results = findAggTimeGreatest(
		...
		ZonedDateTime date_a = ZonedDateTime.of(2020, 6, 1, 21, 0, 0, 0, ZoneOffset.UTC);
		assertAggregateDatum("Greatest result returned A", datum_a,
