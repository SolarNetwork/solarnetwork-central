/* ==================================================================
 * AuditDatumBizSecurityContractTests.java - 22/09/2026 2:19:25 pm
 *
...
/**
 * Verify the {@link AuditDatumBizSecurityContract} for
 * {@link AuditDatumBiz} with the {@code AuditDatumSecurityAspect} aspect
 * applied.
 *
 * @author matt
 * @version 1.0
...
public class AuditDatumBizSecurityContractTests {

	private final TestTenants tenants = new TestTenants();

	private SecuredProxy<AuditDatumBiz> proxy() {
		return securedProxy((AuditDatumBiz) mock(DaoAuditDatumBiz.class),
				new AuditDatumSecurityAspect(tenants.ownershipDao()));
	}

	@TestFactory
	public Stream<DynamicNode> securityContract() {
		return AuditDatumBizSecurityContract.contract(tenants).dynamicTests(this::proxy);
	}

	@Test
	public void pointcutsMatch() {
		// @formatter:off
		then(unmatchedPointcuts(AuditDatumSecurityAspect.class, DaoAuditDatumBiz.class))
			.as("Every AuditDatumSecurityAspect pointcut matches a AuditDatumBiz method")
			.isEmpty()
			;
		// @formatter:on
	}
