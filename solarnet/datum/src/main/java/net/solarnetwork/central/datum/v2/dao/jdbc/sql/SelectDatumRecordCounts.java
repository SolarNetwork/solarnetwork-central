 ...
 * @since 3.8
 */
public final class SelectDatumRecordCounts implements PreparedStatementCreator, SqlProvider {
	...
	private static String colAlias(Aggregation agg) {
		return switch (agg) {
			case Hour -> "datum_hourly_count";
			case Day -> "datum_daily_count";
			case Month -> "datum_monthly_count";
			default -> "datum_count";
		};
	}
	...
	private static String table(Aggregation agg) {
		return switch (agg) {
			case Hour -> "agg_datm_hourly";
			case Day -> "agg_datm_daily";
			case Month -> "agg_datm_monthly";
			default -> "da_datm";
		};
	}
