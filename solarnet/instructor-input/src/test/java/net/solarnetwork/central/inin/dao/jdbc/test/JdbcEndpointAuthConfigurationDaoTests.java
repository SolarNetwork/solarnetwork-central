...
import static net.solarnetwork.central.inin.dao.jdbc.test.InstructionInputJdbcTestUtils.allEndpointAuthConfigurationData;
	...
	public void findForUser() throws Exception {
		...
		for ( int i = 0; i < count; i++ ) {
			for ( int u = 0; u < userCount; u++ ) {
				...
				CredentialConfiguration cred = InstructionInputJdbcTestUtils
						.newCredentialConfiguration(userId, randomString(), randomString());
				cred = credentialDao.get(credentialDao.save(cred));
				EndpointConfiguration endpoint = newEndpointConfiguration(userId, UUID.randomUUID(),
						randomString(), new Long[] { randomLong() }, null, null);
				endpoint = endpointDao.get(endpointDao.save(endpoint));
