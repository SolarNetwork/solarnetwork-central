...
public class SelectCapacityProviderConfiguration
	...
	private void sqlWhere(StringBuilder buf) {
		...
		if ( filter.hasUserCriteria() ) {
			idx += whereOptimizedArrayContains(filter.getUserIds(), "uocc.user_id", where);
		}
		if ( filter.hasConfigurationCriteria() ) {
			idx += whereOptimizedArrayContains(filter.getConfigurationIds(), "uocc.id", where);
		}
		if ( idx > 0 ) {
	...
	private void sqlOrderBy(StringBuilder buf) {
		buf.append("ORDER BY uocc.user_id,uocc.id");
	}
