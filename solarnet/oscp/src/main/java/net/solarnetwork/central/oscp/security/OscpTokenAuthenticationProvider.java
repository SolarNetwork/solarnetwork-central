...
public class OscpTokenAuthenticationProvider implements AuthenticationProvider {
	...
	public @Nullable Authentication authenticate(Authentication authentication)
		...
		if ( principal == null ) {
			throw new BadCredentialsException("Principal (token) missing.");
		}
		...
		var details = new OscpAuthenticatedToken(info);
		var result = new PreAuthenticatedAuthenticationToken(token, "N/A", details.getAuthorities());
