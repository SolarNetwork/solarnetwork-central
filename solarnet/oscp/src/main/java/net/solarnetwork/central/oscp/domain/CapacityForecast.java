 ...
 * @version 1.0
 */
public record CapacityForecast(@Nullable ForecastType type, @Nullable List<TimeBlockAmount> blocks) {
	 ...
	 * @return the instance, or {@code null} if {@code update} is {@code null}
	 */
	public static @Nullable CapacityForecast forOscp20Value(
			@Nullable UpdateGroupCapacityForecast update) {
		if ( update == null ) {
			return null;
		}
	 ...
	 * @param adjust
	 *        the OSCP 2.0 value to get an instance for
	 * @return the instance, or {@code null} if {@code adjust} is {@code null}
	 */
	public static @Nullable CapacityForecast forOscp20Value(
			@Nullable AdjustGroupCapacityForecast adjust) {
		if ( adjust == null ) {
			return null;
		}
		ForecastType type = (adjust.getType() != null ? ForecastType.forOscp20Value(adjust.getType())
				: null);
		List<TimeBlockAmount> blocks = (adjust.getForecastedBlocks() != null ? adjust
				.getForecastedBlocks().stream().map(TimeBlockAmount::forOscp20ForecastValue).toList()
				: null);
		return new CapacityForecast(type, blocks);
	}
