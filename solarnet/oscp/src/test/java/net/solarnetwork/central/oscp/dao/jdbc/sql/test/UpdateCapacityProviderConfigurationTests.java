...
public class UpdateCapacityProviderConfigurationTests {
	...
	public void sql() {
		...
		// WHEN
		String sql = new UpdateCapacityProviderConfiguration(conf.getId(), conf).getSql();

		// THEN
		log.debug("Generated SQL:\n{}", sql);
		assertThat("SQL matches", sql, equalToTextResource("update-capacity-provider-conf.sql",
	...
	public void prep() throws SQLException {
		...
		// WHEN
		PreparedStatement result = new UpdateCapacityProviderConfiguration(conf.getId(), conf)
		...
		assertThat("Generated SQL", sqlCaptor.getValue(), equalToTextResource(
				"update-capacity-provider-conf.sql", TestSqlResources.class, SQL_COMMENT));
