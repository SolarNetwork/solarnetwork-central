/* ==================================================================
 * SelectAssetConfigurationTests.java - 12/08/2022 5:02:05 pm
 * 
...
/**
 * Test cases for the {@link SelectAssetConfiguration} class.
 * 
 * @author matt
 * @version 1.0
 */
@ExtendWith(MockitoExtension.class)
public class SelectAssetConfigurationTests {
	...
	private void givenSetGroupIdsArrayParameter(Long[] value) throws SQLException {
		given(con.createArrayOf(eq("bigint"), aryEq(value))).willReturn(groupIdsArray);
	}

	private void thenPrepStatement(PreparedStatement result, AssetFilter filter) throws SQLException {
		int p = 0;
		...
		if ( filter.hasGroupCriteria() ) {
			if ( filter.getGroupIds().length == 1 ) {
				then(result).should().setObject(++p, filter.getGroupId());
			} else {
				then(result).should().setArray(++p, groupIdsArray);
			}
		}
	...
	public void multi_sql() {
		...
		// WHEN
		String sql = new SelectAssetConfiguration(filter).getSql();

		// THEN
		log.debug("Generated SQL:\n{}", sql);
		assertThat("SQL matches", sql,
				equalToTextResource("select-asset-conf.sql", TestSqlResources.class, SQL_COMMENT));
	}
	...
	public void multi_prep() throws SQLException {
		...
		// WHEN
		PreparedStatement result = new SelectAssetConfiguration(filter).createPreparedStatement(con);
	...
	public void single_sql() {
		...
		log.debug("Generated SQL:\n{}", sql);
		assertThat("SQL matches", sql,
				equalToTextResource("select-asset-conf-one.sql", TestSqlResources.class, SQL_COMMENT));
	}
	...
	public void group_sql() {
		...
		// THEN
		log.debug("Generated SQL:\n{}", sql);
		assertThat("SQL matches", sql, equalToTextResource("select-asset-conf-group-one.sql",
				TestSqlResources.class, SQL_COMMENT));
	}

	@Test
	public void group_prep() throws SQLException {
		// GIVEN
		BasicConfigurationFilter filter = new BasicConfigurationFilter();
		filter.setUserId(1L);
		filter.setGroupId(2L);
		...
		assertThat("Generated SQL", sqlCaptor.getValue(), equalToTextResource(
				"select-asset-conf-group-one.sql", TestSqlResources.class, SQL_COMMENT));
		assertThat("Connection statement returned", result, sameInstance(stmt));
		thenPrepStatement(result, filter);
	}

	@Test
	public void group_multi_sql() {
		// GIVEN
		BasicConfigurationFilter filter = new BasicConfigurationFilter();
		filter.setUserIds(new Long[] { 1L, 2L });
		filter.setGroupIds(new Long[] { 3L, 4L });
		...
		log.debug("Generated SQL:\n{}", sql);
		assertThat("SQL matches", sql,
				equalToTextResource("select-asset-conf-group.sql", TestSqlResources.class, SQL_COMMENT));
	}

	@Test
	public void group_multi_prep() throws SQLException {
		// GIVEN
		BasicConfigurationFilter filter = new BasicConfigurationFilter();
		filter.setUserIds(new Long[] { 1L, 2L });
		filter.setGroupIds(new Long[] { 3L, 4L });
		...
		givenPrepStatement();
		givenSetUserIdsArrayParameter(filter.getUserIds());
		givenSetGroupIdsArrayParameter(filter.getGroupIds());
