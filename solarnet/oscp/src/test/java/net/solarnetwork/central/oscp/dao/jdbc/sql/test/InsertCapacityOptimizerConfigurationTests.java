...
public class InsertCapacityOptimizerConfigurationTests {
	...
	public void assigned_sql() {
		...
		// THEN
		log.debug("Generated SQL:\n{}", sql);
		assertThat("SQL matches", sql, equalToTextResource("insert-capacity-optimizer-conf-assigned.sql",
				TestSqlResources.class, SQL_COMMENT));
	}
	...
	public void assigned_prep() throws SQLException {
		...
		log.debug("Generated SQL:\n{}", sqlCaptor.getValue());
		assertThat("Generated SQL", sqlCaptor.getValue(), equalToTextResource(
				"insert-capacity-optimizer-conf-assigned.sql", TestSqlResources.class, SQL_COMMENT));
		assertThat("Statement returned", result, sameInstance(stmt));
		thenPrepStatement(result, userId, conf);
	}
