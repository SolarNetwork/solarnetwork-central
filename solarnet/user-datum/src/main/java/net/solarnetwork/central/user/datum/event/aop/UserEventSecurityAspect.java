...
public class UserEventSecurityAspect extends AuthorizationSupport {
	...
	@Pointcut("execution(* net.solarnetwork.central.user.datum.event.biz.*Biz.*ForUser(..)) && args(userId,..)")
	public void actionForUser(Long userId) {
	}

	@Pointcut("execution(* net.solarnetwork.central.user.datum.event.biz.*Biz.save*(..)) && args(config,..)")
	public void saveConfiguration(UserIdRelated config) {
	}

	@Pointcut("execution(* net.solarnetwork.central.user.datum.event.biz.*Biz.delete*(..)) && args(config,..)")
