/* ==================================================================
 * UserExpireBizSecurityContractTests.java - 22/09/2026 2:19:25 pm
 *
...
import net.solarnetwork.central.user.datum.expire.biz.dao.DaoUserExpireBiz;
import net.solarnetwork.central.user.datum.expire.biz.dao.DaoUserDatumDeleteBiz;

/**
 * Verify the {@link UserExpireBizSecurityContract} for {@link UserExpireBiz}
 * with the {@code UserExpireSecurityAspect} aspect applied.
 *
 * @author matt
 * @version 1.0
 */
@ExtendWith(SecurityContextExtension.class)
public class UserExpireBizSecurityContractTests {

	private final TestTenants tenants = new TestTenants();

	private SecuredProxy<UserExpireBiz> proxy() {
		return securedProxy((UserExpireBiz) mock(DaoUserExpireBiz.class),
				new UserExpireSecurityAspect(tenants.ownershipDao()));
	}

	@TestFactory
	public Stream<DynamicNode> securityContract() {
		return UserExpireBizSecurityContract.contract(tenants).dynamicTests(this::proxy);
	}

	@Test
	public void pointcutsMatch() {
		// @formatter:off
		then(unmatchedPointcuts(UserExpireSecurityAspect.class, DaoUserExpireBiz.class,
				DaoUserDatumDeleteBiz.class))
			.as("Every UserExpireSecurityAspect pointcut matches a UserExpireBiz or UserDatumDeleteBiz method")
			.isEmpty()
			;
		// @formatter:on
	}
