...
public class SelectServerConfiguration
	 ...
	 *        the filter
	 */
	public SelectServerConfiguration(ServerFilter filter) {
	 ...
	 *        the filter
	 */
	public SelectServerConfiguration(ServerFilter filter, int fetchSize) {
		super();
	...
	private void sqlCore(StringBuilder buf) {
		buf.append("""
				SELECT ds.user_id, ds.id, ds.created, ds.modified, ds.enabled
					, ds.cname
				FROM solardnp3.dnp3_server ds
				""");
	}
	...
	private void sqlWhere(StringBuilder buf) {
		...
		if ( filter.hasUserCriteria() ) {
			idx += whereOptimizedArrayContains(filter.getUserIds(), "ds.user_id", where);
		}
		if ( filter.hasServerCriteria() ) {
			idx += whereOptimizedArrayContains(filter.getServerIds(), "ds.id", where);
		}
		if ( filter.hasEnabledCriteria() ) {
			where.append("\tAND ds.enabled = ?\n");
			idx += 1;
		}
		if ( idx > 0 ) {
	...
	private void sqlOrderBy(StringBuilder buf) {
		buf.append("ORDER BY ds.user_id, ds.id");
	}
