...
/**
 * Update the enabled flag of server data point-related entities.
 *
 * @author matt
 * @version 1.0
 */
public final class UpdateEnabledServerDataPointFilter implements PreparedStatementCreator, SqlProvider {
	...
	private void sqlWhere(StringBuilder where) {
		...
		if ( filter.hasSourceCriteria() ) {
			whereOptimizedArrayContains(filter.getSourceIds(), "source_id", where);
		}
