 ...
 * @version 1.0
 */
public class UpdateCloudIntegrationOAuthAuthorizationState implements PreparedStatementCreator, SqlProvider {
	 ...
	 */
	public UpdateCloudIntegrationOAuthAuthorizationState(UserLongCompositePK id, String state, String expectedState) {
	...
	public String getSql() {
		...
		buf.append("""
				...
				""");
		if ( state == null ) {
			buf.append("sprops - 'oauthState'\n");
		} else {
			buf.append(
					"jsonb_set(COALESCE(sprops, '{}'::jsonb), '{oauthState}', to_jsonb(?::TEXT), TRUE)\n");
		}
		buf.append("""
				WHERE user_id = ?
				AND id = ?
		...
		if ( expectedState != null ) {
			buf.append("AND sprops ->> 'oauthState' = ?");
		}
		return buf.toString();
	}
