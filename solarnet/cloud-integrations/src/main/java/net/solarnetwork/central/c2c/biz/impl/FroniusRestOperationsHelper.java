...
import static net.solarnetwork.central.c2c.biz.impl.FroniusCloudIntegrationService.API_KEY_HEADER;
import static net.solarnetwork.central.c2c.biz.impl.FroniusCloudIntegrationService.API_KEY_SETTING;
import static net.solarnetwork.central.c2c.biz.impl.FroniusCloudIntegrationService.API_SECRET_HEADER;
import static net.solarnetwork.central.c2c.biz.impl.FroniusCloudIntegrationService.API_SECRET_SETTING;
	...
	public <B, R, C extends CloudIntegrationsConfigurationEntity<C, K>, K extends UserRelatedCompositeKey<K>, T> T http(
		...
		return super.http(description, method, body, configuration, responseType, (headers) -> {
			if ( configuration instanceof IdentifiableConfiguration c
				...
				final String apiKey = nonEmptyString(((IdentifiableConfiguration) decrypted)
						.serviceProperty(API_KEY_SETTING, String.class));
				final String apiSecret = nonEmptyString(((IdentifiableConfiguration) decrypted)
						.serviceProperty(API_SECRET_SETTING, String.class));
				if ( apiKey != null && apiSecret != null ) {
					headers.add(API_KEY_HEADER, apiKey);
					headers.add(API_SECRET_HEADER, apiSecret);
				}
			}
