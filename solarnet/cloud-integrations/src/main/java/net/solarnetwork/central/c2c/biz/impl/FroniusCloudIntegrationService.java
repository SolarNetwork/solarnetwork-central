...
public class FroniusCloudIntegrationService extends BaseRestOperationsCloudIntegrationService {
	...
	/**
	 * The URL template for listing all available PV systems.
	 */
	public static final String LIST_PVSYSTEMS_URL = "/pvsystems";
	...
	public static final String API_KEY_HEADER = "AccessKeyId";

	/** The API secret authorization HTTP header name. */
	public static final String API_SECRET_HEADER = "AccessKeyValue";
	...
	public Result<Void> validate(CloudIntegrationConfiguration integration, Locale locale) {
		...
		final String apiKey = nonEmptyString(integration.serviceProperty(API_KEY_SETTING, String.class));
		if ( apiKey == null ) {
		...
		final String apiSecret = nonEmptyString(
				integration.serviceProperty(API_SECRET_SETTING, String.class));
		if ( apiSecret == null ) {
			String errMsg = ms.getMessage("error.apiSecret.missing", null, locale);
			errorDetails.add(new ErrorDetail(API_SECRET_SETTING, null, errMsg));
		}
		...
		try {
			final String response = restOpsHelper.httpGet("List systems", integration, String.class,
					(req) -> UriComponentsBuilder.fromUri(resolveBaseUrl(integration, BASE_URI))
							.path(FroniusCloudIntegrationService.LIST_PVSYSTEMS_URL).buildAndExpand()
							.toUri(),
					res -> res.getBody());
