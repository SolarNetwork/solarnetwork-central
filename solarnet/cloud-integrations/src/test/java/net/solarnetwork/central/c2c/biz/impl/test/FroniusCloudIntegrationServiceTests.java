...
public class FroniusCloudIntegrationServiceTests {
				...
				and.then(r.getErrors())
					...
					.satisfies(errors -> {
						...
						and.then(errors)
							.as("Error detail")
							.element(1)
							.as("API secret flagged")
							.returns(API_SECRET_SETTING, from(ErrorDetail::getLocation))
							;
					})
					;
			})
	...
	public void validate_ok() {
		...
		// @formatter:off
		conf.setServiceProps(Map.of(
				FroniusCloudIntegrationService.API_KEY_SETTING, apiKey,
				FroniusCloudIntegrationService.API_SECRET_SETTING, apiSecret
		));
		// @formatter:on

		final URI listSystemsUri = FroniusCloudIntegrationService.BASE_URI
				.resolve(FroniusCloudIntegrationService.LIST_PVSYSTEMS_URL);
		...
		and.then(httpEntityCaptor.getValue().getHeaders())
			...
			.containsEntry(FroniusCloudIntegrationService.API_KEY_HEADER, List.of(apiKey))
			.as("Request headers contains API secret")
			.containsEntry(FroniusCloudIntegrationService.API_SECRET_HEADER, List.of(apiSecret))
			;
