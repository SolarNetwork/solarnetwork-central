...
public class AsyncChargePointStatusDaoTests {
	...
	private MutableClock clock = MutableClock.of(Instant.now().truncatedTo(ChronoUnit.SECONDS),
			ZoneOffset.UTC);
	private NavigableSet<StatusUpdate> statuses;
	private AsyncChargePointStatusDao dao;

	@BeforeEach
	public void setup() {
		statuses = new ConcurrentSkipListSet<>();
	...
	public void updateStatus_connected() {
		...
		// THEN
		// @formatter:off
		then(scheduler).should().schedule(same(dao), eq(clock.instant().plus(dao.getFlushDelay())));
		and.then(statuses)
			.as("Status buffered")
			.hasSize(1)
			.first()
			.as("User ID same as passed in method")
			.returns(userId, from(StatusUpdate::userId))
			.as("Charge point ID same as passed in method")
			.returns(chargePointIdentifier, from(StatusUpdate::chargePointIdentifier))
			.as("Connected to same as passed in method")
			.returns(connectedTo, from(StatusUpdate::connectedTo))
			.as("Session ID same as passed in method")
			.returns(sessionId, from(StatusUpdate::sessionId))
			.as("Connection date same as passed in method")
			.returns(connectionDate, from(StatusUpdate::connectionDate))
			.as("Connected flag same as passed in method")
			.returns(connected, from(StatusUpdate::connected))
			;
		// @formatter:on
	}
	...
	public void flush_notLast() {
		...
		statuses.add(new StatusUpdate(clock.instant().plusSeconds(1), userId2, chargePointIdentifier2,
		...
		and.then(statuses)
			...
			.first()
			.as("User ID same as passed in method")
			.returns(userId2, from(StatusUpdate::userId))
			.as("Charge point ID same as passed in method")
			.returns(chargePointIdentifier2, from(StatusUpdate::chargePointIdentifier))
			.as("Connected to same as passed in method")
			.returns(connectedTo2, from(StatusUpdate::connectedTo))
			.as("Session ID same as passed in method")
			.returns(sessionId2, from(StatusUpdate::sessionId))
			.as("Connection date same as passed in method")
			.returns(connectionDate2, from(StatusUpdate::connectionDate))
			.as("Connected flag same as passed in method")
			.returns(connected2, from(StatusUpdate::connected))
			;
		// @formatter:on
