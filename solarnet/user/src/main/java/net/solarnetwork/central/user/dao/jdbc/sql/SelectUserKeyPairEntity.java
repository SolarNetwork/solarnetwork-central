/* ==================================================================
 * SelectUserKeyPairEntity.java - 22/03/2025 7:17:56 am
 *
...
/**
 * Support for SELECT for {@link UserKeyPairEntity} entities.
 *
 * @author matt
 * @version 1.0
 */
public class SelectUserKeyPairEntity
		implements PreparedStatementCreator, SqlProvider, CountPreparedStatementCreatorProvider {

	/** The {@code fetchSize} property default value. */
	public static final int DEFAULT_FETCH_SIZE = 100;

	private final UserKeyPairFilter filter;
	 ...
	 */
	public SelectUserKeyPairEntity(UserKeyPairFilter filter) {
	 ...
	 *         if any argument is {@code null}
	 */
	public SelectUserKeyPairEntity(UserKeyPairFilter filter, int fetchSize) {
		super();
	...
	private void sqlCore(StringBuilder buf) {
		buf.append("""
				SELECT uk.user_id, uk.skey, uk.created, uk.modified, uk.keystore
				FROM solaruser.user_keypair uk
				""");
	}
	...
	private void sqlWhere(StringBuilder buf) {
		...
		if ( filter.hasUserCriteria() ) {
			idx += whereOptimizedArrayContains(filter.getUserIds(), "uk.user_id", where);
		}
		if ( filter.hasKeyCriteria() ) {
			idx += whereOptimizedArrayContains(filter.getKeys(), "uk.skey", where);
		}
		if ( idx > 0 ) {
	...
	private void sqlOrderBy(StringBuilder buf) {
		buf.append("ORDER BY uk.user_id, uk.skey");
	}
