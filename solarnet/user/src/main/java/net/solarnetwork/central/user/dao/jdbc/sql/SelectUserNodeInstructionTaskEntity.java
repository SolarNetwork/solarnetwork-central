/* ==================================================================
 * SelectUserNodeInstructionTaskEntity.java - 10/11/2025 4:34:09 pm
 *
...
/**
 * Support for SELECT for {@link UserNodeInstructionTaskEntity} entities.
 *
 * @author matt
 * @version 1.0
 */
public class SelectUserNodeInstructionTaskEntity
	 ...
	 *        the filter
	 */
	public SelectUserNodeInstructionTaskEntity(UserNodeInstructionTaskFilter filter) {
	 ...
	 *        the filter
	 */
	public SelectUserNodeInstructionTaskEntity(UserNodeInstructionTaskFilter filter, int fetchSize) {
		super();
	...
	private void sqlCore(StringBuilder buf) {
		buf.append(
				"""
						SELECT unit.user_id, unit.id, unit.enabled
							, unit.cname, unit.node_id, unit.topic, unit.schedule, unit.status, unit.exec_at, unit.sprops
							, unit.last_exec_at, unit.message, unit.rprops
						FROM solaruser.user_node_instr_task unit
						""");
	}
	...
	private void sqlWhere(StringBuilder buf) {
		...
		if ( filter.hasUserCriteria() ) {
			idx += whereOptimizedArrayContains(filter.getUserIds(), "unit.user_id", where);
		}
		if ( filter.hasNodeCriteria() ) {
			idx += whereOptimizedArrayContains(filter.getNodeIds(), "unit.node_id", where);
		}
		if ( filter.hasTopicCriteria() ) {
			idx += whereOptimizedArrayContains(filter.getTopics(), "unit.topic", where);
		}
		if ( filter.hasTaskCriteria() ) {
			idx += whereOptimizedArrayContains(filter.getTaskIds(), "unit.id", where);
		}
		if ( filter.hasClaimableJobStateCriteria() ) {
			idx += whereOptimizedArrayContains(filter.claimableJobStateKeys(), "unit.status", where);
		}
		if ( idx > 0 ) {
	...
	private void sqlOrderBy(StringBuilder buf) {
		buf.append("ORDER BY unit.user_id, unit.id");
	}
