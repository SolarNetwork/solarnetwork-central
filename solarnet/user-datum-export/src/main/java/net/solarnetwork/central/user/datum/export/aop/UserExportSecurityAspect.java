...
public class UserExportSecurityAspect extends AuthorizationSupport {
	...
	@Pointcut("execution(* net.solarnetwork.central.user.datum.export.biz.UserExportBiz.*ForUser(..)) && args(userId,..)")
	public void actionForUser(Long userId) {
	}

	@Pointcut("execution(* net.solarnetwork.central.user.datum.export.biz.UserExportBiz.save*(..)) && args(config,..)")
	public void saveConfiguration(UserRelatedEntity<?> config) {
	}

	@Pointcut("execution(* net.solarnetwork.central.user.datum.export.biz.UserExportBiz.delete*(..)) && args(config,..)")
