/* ==================================================================
 * DatumImportBizSecurityContractTests.java - 22/09/2026 2:19:25 pm
 *
...
/**
 * Verify the {@link DatumImportBizSecurityContract} for {@link DatumImportBiz}
 * with the {@code DatumImportSecurityAspect} aspect applied.
 *
 * @author matt
 * @version 1.0
 */
@ExtendWith(SecurityContextExtension.class)
public class DatumImportBizSecurityContractTests {

	private final TestTenants tenants = new TestTenants();

	private SecuredProxy<DatumImportBiz> proxy() {
		return securedProxy((DatumImportBiz) mock(DaoDatumImportBiz.class),
				new DatumImportSecurityAspect(tenants.ownershipDao()));
	}

	@TestFactory
	public Stream<DynamicNode> securityContract() {
		return DatumImportBizSecurityContract.contract(tenants).dynamicTests(this::proxy);
	}

	@Test
	public void pointcutsMatch() {
		// @formatter:off
		then(unmatchedPointcuts(DatumImportSecurityAspect.class, DaoDatumImportBiz.class))
			.as("Every DatumImportSecurityAspect pointcut matches a DatumImportBiz method")
			.isEmpty()
			;
		// @formatter:on
	}
