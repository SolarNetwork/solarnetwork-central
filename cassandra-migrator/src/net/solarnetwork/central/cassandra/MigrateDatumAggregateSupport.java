 ...
 * @version 1.0
 */
public abstract class MigrateDatumAggregateSupport extends MigrateLocationDatumSupport {

	public static final String DEFAULT_CQL = "INSERT INTO solardata.node_datum_agg (node_id, dtype, source_id, atype, year, ts, data_num) "
			+ "VALUES (?, ?, ?, ?, ?, ?, ?)";

	/**
	 * Default constructor.
	 */
	public MigrateDatumAggregateSupport() {
		super();
		setCql(DEFAULT_CQL);
	}
	 ...
	 * @param other
	 */
	public MigrateDatumAggregateSupport(MigrateDatumSupport other) {
		super(other);
	}
	...
	protected BoundStatement getBoundStatementForResultRowMapping(ResultSet rs,
		...
		bs.setInt(3, getAggregateType());

		Timestamp created = rs.getTimestamp(3);
		gmtCalendar.setTimeInMillis(created.getTime());
		bs.setInt(4, gmtCalendar.get(Calendar.YEAR));

		bs.setDate(5, created);
		return bs;
	}
