 ...
 * @version 1.0
 */
public class MigratePriceDatumAggregateDaily extends MigrateLocationDatumAggregateSupport {

	private static final String SQL = "SELECT loc_id, created_day, price FROM solarrep.rep_price_datum_daily "
			+ "WHERE loc_id = ? AND created_day >= ? AND created_day < ? ORDER BY loc_id, created_day";

	private static final String COUNT_SQL = "SELECT count(loc_id) FROM solarrep.rep_price_datum_daily";

	/**
	 * Default constructor.
	 */
	public MigratePriceDatumAggregateDaily() {
		super();
		setSql(SQL);
		setCountSql(COUNT_SQL);
		setupGroupedDateRangeSql("solarrep.rep_price_datum_daily", "loc_id", "created_day");
	}
	 ...
	 * @param other
	 */
	public MigratePriceDatumAggregateDaily(MigrateDatumSupport other) {
		super(other);
	}
