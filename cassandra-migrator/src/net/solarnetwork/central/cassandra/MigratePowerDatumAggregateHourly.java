 ...
 * @version 1.0
 */
public class MigratePowerDatumAggregateHourly extends MigrateDatumAggregateSupport {

	private static final String SQL = "SELECT node_id, source_id, created_hour, watts, watt_hours, bat_volts, cost_amt, cost_currency FROM solarrep.rep_power_datum_hourly "
			+ "WHERE node_id = ? AND created_hour >= ? AND created_hour < ? ORDER BY node_id, created_hour";

	private static final String COUNT_SQL = "SELECT count(node_id) FROM solarrep.rep_power_datum_hourly";

	/**
	 * Default constructor.
	 */
	public MigratePowerDatumAggregateHourly() {
		super();
		setSql(SQL);
		setCountSql(COUNT_SQL);
		setupGroupedDateRangeSql("solarrep.rep_power_datum_hourly", "node_id", "created_hour");
	}
	 ...
	 * @param other
	 */
	public MigratePowerDatumAggregateHourly(MigrateDatumSupport other) {
		super(other);
	}
