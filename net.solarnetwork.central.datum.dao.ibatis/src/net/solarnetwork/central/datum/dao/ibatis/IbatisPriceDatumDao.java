/* ===================================================================
 * IbatisPriceDatumDao.java
 * 
 * Created Aug 7, 2009 5:47:04 PM
 * 
...
/**
 * iBATIS implementation of {@link PriceDatumDao}.
 *
 * @author matt
 * @version $Revision$ $Date$
 */
public class IbatisPriceDatumDao extends IbatisDatumDaoSupport<PriceDatum>
implements PriceDatumDao {

	/**
	 * The query name used in {@link #setupAggregatedDatumQuery(DatumQueryCommand, Map)}
	 * for minute-precise results.
	 */
	public static final String QUERY_PRICE_DATUM_FOR_AGGREGATE_BY_MINUTE 
		= "find-PriceDatum-for-agg-minute";
	
	/** 
	 * The query name used in {@link #setupAggregatedDatumQuery(DatumQueryCommand, Map)}
	 * for hour-precise results.
	 */
	public static final String QUERY_PRICE_DATUM_FOR_AGGREGATE_BY_HOUR 
		= "find-PriceDatum-for-agg-hour";
	
	/** 
	 * The query name used in {@link #setupAggregatedDatumQuery(DatumQueryCommand, Map)}
	 * for day-precise results.
	 */
	public static final String QUERY_PRICE_DATUM_FOR_AGGREGATE_BY_DAY 
		= "find-PriceDatum-for-agg-day";
	
	/** 
	 * The query name used in {@link #setupAggregatedDatumQuery(DatumQueryCommand, Map)}
	 * for week-precise results.
	 */
	public static final String QUERY_PRICE_DATUM_FOR_AGGREGATE_BY_WEEK 
		= "find-PriceDatum-for-agg-week";
	
	/** 
	 * The query name used in {@link #setupAggregatedDatumQuery(DatumQueryCommand, Map)}
	 * for month-precise results.
	 */
	public static final String QUERY_PRICE_DATUM_FOR_AGGREGATE_BY_MONTH 
		= "find-PriceDatum-for-agg-month";
		...
		if ( criteria.getLocationId() == null ) {
			...
			if ( log.isWarnEnabled() ) {
				log.warn("PriceDatum query does not provide location ID, "
						+"falling back to hack to first available location. Query params: " 
						+params);
			}
			...
			PriceDatum min = (PriceDatum)getSqlMapClientTemplate().queryForObject(
					QUERY_PRICE_DATUM_MIN, minPriceLocParams);
			if ( min == null ) {
				if ( log.isDebugEnabled() ) {
					log.debug("Minimum PriceDatum not found from query ["
						+QUERY_PRICE_DATUM_MIN +"] and parameters "
						+minPriceLocParams);
				}
				return null;
			}
	...
	@Override
	protected List<PriceDatum> postProcessDatumQuery(
			DatumQueryCommand criteria, Map<String, Object> params,
			List<PriceDatum> results) {
		...
		/* FIXME
		PriceDatum min = (PriceDatum)getSqlMapClientTemplate().queryForObject(
				QUERY_PRICE_DATUM_MIN_FOR_NODE, params);
		if ( min == null ) {
			if ( log.isDebugEnabled() ) {
				log.debug("Minimum PriceDatum not found from query ["
		...
		for ( PriceDatum pd : results ) {
			pd.setCurrency(min.getCurrency());
