...
/**
 * Implementation of {@link DataCollectorBiz} using {@link DatumDao} objects
 * to persist the data.
 * 
 * <p>This service expects all calls into {@link #postDatum(Datum)} and
 * {@link #postDatum(Iterable)} to provide a {@link AuthenticatedNode}
 * via the normal Spring Security {@link SecurityContextHolder} API. Any
 * attempt to post data for a node different from the currently authenticated
 * node will result in a {@link SecurityException}. If a {@link NodeDatum}
 * is posted with a <em>null</em> {@link NodeDatum#getNodeId()} value, this
 * service will set the node ID to the authenticated node ID automatically.</p>
 * 
 * <p>The configurable properties of this class are:</p>
 * 
 * <dl class="class-properties">
 *   <dt>daoMapping</dt>
 *   <dd>A mapping of {@link Datum} class objects to associated 
 *   {@link DatumDao} objects that know how to persist them.</dd>
 *   
 *   <dt>solarNodeDao</dt>
 ...
 *   that information when passed to {@link #postDatum(Datum)}.</dd>
 * </dl>
 * 
 * @author matt
 * @version $Id$
...
public class DaoDataCollectorBiz implements DataCollectorBiz {
	...
	private static final Set<String> CONSUMPTION_DATUM_IGNORE_ENTITY_COPY 
		= Collections.unmodifiableSet(new HashSet<String>(Arrays.asList(
			new String[] {"class", "id", "created", "nodeId", "watts"})));

	private static final Set<String> POWER_DATUM_IGNORE_ENTITY_COPY 
		= Collections.unmodifiableSet(new HashSet<String>(Arrays.asList(
			new String[] {"class", "id", "created", "nodeId"})));

	private static final Set<String> PRICE_DATUM_IGNORE_ENTITY_COPY 
	= Collections.unmodifiableSet(new HashSet<String>(Arrays.asList(
			new String[] {"class", "id", "created", "locationId"})));


	private Map<Class<? extends Datum>, DatumDao<Datum>> daoMapping;
	...
	public <D extends Datum> D postDatum(D datum) {
		...
		if ( authNode != null && datum instanceof NodeDatum ) {
			...
			if ( nd.getNodeId() == null ) {
				...
				if ( log.isWarnEnabled() ) {
					log.warn("Illegal datum post by node " +authNode.getNodeId()
							+" as node " +nd.getNodeId());
				}
				throw new SecurityException("Illegal data access");
			}
		}

		DatumDao<Datum> dao = daoMapping.get(datum.getClass());
		if ( dao == null ) {
			throw new IllegalArgumentException("The [" 
					+datum.getClass().getName() +"] Datum is not supported.");
		}
		...
		try {
			...
			if ( log.isDebugEnabled() ) {
				log.debug("DataIntegretyViolation on store of "
	...
	public List<SourceLocationMatch> findWeatherLocation(SourceLocation criteria) {
		FilterResults<SourceLocationMatch> matches = weatherLocationDao.findFiltered(
	...
	public Datum preprocessConsumptionDatum(ConsumptionDatum datum, 
			DatumDao<? extends Datum> dao) {
	...
	public Datum preprocessDayDatum(DayDatum datum, DayDatumDao dao) {
		...
		DayDatum entity = dao.getDatumForDate(
				datum.getNodeId(), datum.getDay());
	...
	public Datum preprocessWeatherDatum(WeatherDatum datum, WeatherDatumDao dao) {
		...
		WeatherDatum entity = dao.getDatumForDate(
				datum.getLocationId(), datum.getInfoDate());
	...
	private Datum checkForNodeDatumByDate(NodeDatum datum, DatumDao<? extends Datum> dao, 
		...
		if ( entity != null ) {
			...
			Map<String, Object> propsToCopy = ClassUtils.getBeanProperties(datum, 
					copyPropertiesIgnore);
	...
	public Map<Class<? extends Datum>, DatumDao<Datum>> getDaoMapping() {
		return daoMapping;
	}
