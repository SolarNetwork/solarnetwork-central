DROP FUNCTION solardatum.find_most_recent(solarcommon.node_ids);
...
DROP FUNCTION solardatum.find_most_recent(solarcommon.node_id, solarcommon.source_ids);
CREATE OR REPLACE FUNCTION solardatum.find_most_recent(
	node bigint,
	sources text[] DEFAULT NULL)
  RETURNS SETOF solardatum.da_datum AS
...
DROP FUNCTION solardatum.store_datum(solarcommon.ts, solarcommon.node_id, solarcommon.source_id, solarcommon.ts, text);
