...
public abstract class AbstractNodeController {
	...
	private String[] requestDateFormats 
		= new String[] {DEFAULT_DATE_TIME_FORMAT, DEFAULT_DATE_FORMAT};
	 ...
	 * 
	 * <p>This sets up a new {@link CloningPropertyEditorRegistrar} as a request attribute,
	 * which could be used by the view for serializing model properties in some way. A
	 * common use for this is to serialize {@link DateTime} objects into Strings, so this
	 * method accepts a {@code dateFormat} and {@code node} property which, if provided,
	 * will add a {@link JodaDateFormatEditor} to the registrar for all {@link DateTime}
	 * objects, configured with the node's time zone.</p>
	 * 
	 * @param request the HTTP request
	 * @param dateFormat an optional date format
	 * @param node an optional node (required if {@code dateFormat} provided)
	...
	protected CloningPropertyEditorRegistrar setupViewPropertyEditorRegistrar(
		...
		if ( dateFormat != null && node != null ) {
			// TODO implement caching of JodaDateFormatEditors based on dateFormat + time zone
			registrar.setPropertyEditor(DateTime.class, new JodaDateFormatEditor(dateFormat, node.getTimeZone()));
		}
	...
	/**
	 * Add a {@link DateTime} property editor, using the {@link #getRequestDateFormat()} pattern.
	 * 
	 * <p>This is typically called from an "init binder" method.</p>
	 * 
	 * @param binder the binder to add the editor to
	 */
	 ...
	 * 
	 * @param requestDateFormat the requestDateFormat to set
	 */
	...
	/**
	 * @param requestDateFormats the requestDateFormats to set
	 */
