...
) RETURNS SETOF TIMESTAMP WITH TIME ZONE LANGUAGE plpgsql STRICT STABLE ROWS 1 AS
$$
BEGIN
	RETURN QUERY
	SELECT ts
...
CREATE OR REPLACE FUNCTION solardatm.datm_has_accumulating_at(
		...
		has_a 		BOOLEAN
	) LANGUAGE plpgsql STRICT STABLE ROWS 1 AS
$$
BEGIN
	RETURN QUERY
...
) RETURNS SETOF solardatm.da_datm LANGUAGE plpgsql STRICT STABLE ROWS 500 AS
$$
BEGIN
	RETURN QUERY
	SELECT *
...
CREATE OR REPLACE FUNCTION solardatm.find_datm_around_ts(
		...
		must_a		BOOLEAN DEFAULT FALSE,
		has_no_a 	BOOLEAN DEFAULT FALSE
	) RETURNS SETOF solardatm.da_datm LANGUAGE SQL STRICT STABLE ROWS 2 AS
$$
