...
BEGIN
	...
	ELSEIF ts < '2025-07-01'::DATE THEN
	...
	ELSE
		RETURN QUERY SELECT *, '2025-07-01'::DATE FROM ( VALUES
			  ('datum-props-in', 		0::BIGINT, 				0.00000575::NUMERIC)
			, ('datum-props-in', 		500000::BIGINT, 		0.00000345::NUMERIC)
			, ('datum-props-in', 		10000000::BIGINT, 		0.00000092::NUMERIC)
			, ('datum-props-in', 		500000000::BIGINT, 		0.00000023::NUMERIC)
			...
			, ('api-data', 				0::BIGINT, 				0.00000020::NUMERIC)
			, ('api-data', 				1000000000::BIGINT, 	0.00000009::NUMERIC)
			, ('api-data', 				10000000000::BIGINT, 	0.00000003::NUMERIC)
			, ('api-data', 				100000000000::BIGINT,	0.000000015::NUMERIC)
		) AS t(min, meter_key, cost);
	END IF;
END
$$;
...
$$
	...
	, usvc AS (
		...
		SELECT
			...
			, (SUM(a.cnt) FILTER (WHERE a.service LIKE 'U%'))::BIGINT AS api_data
	...
	SELECT
		...
		, LEAST(GREATEST(CASE meter_key
			...
			WHEN 'api-data' THEN usvc.api_data
...
CREATE OR REPLACE FUNCTION solarbill.billing_usage_details(userid BIGINT, ts_min TIMESTAMP, ts_max TIMESTAMP, effective_date DATE DEFAULT CURRENT_DATE)
	RETURNS TABLE(
		...
		api_data						BIGINT,
		api_data_cost					NUMERIC,
		api_data_tiers					NUMERIC[],
		api_data_tiers_cost				NUMERIC[]
	) LANGUAGE sql STABLE AS
$$
	WITH tier_costs AS (
	...
	SELECT
		...
		, SUM(CASE meter_key WHEN 'api-data' THEN total_count ELSE NULL END)::BIGINT AS api_data
		, SUM(CASE meter_key WHEN 'api-data' THEN total_cost ELSE NULL END) AS api_data_cost
		, solarcommon.first(CASE meter_key WHEN 'api-data' THEN tier_counts ELSE NULL END) AS api_data_tiers
		, solarcommon.first(CASE meter_key WHEN 'api-data' THEN tier_costs ELSE NULL END) AS api_data_tiers_cost
	FROM costs
	HAVING
		...
		OR SUM(CASE meter_key WHEN 'api-data' THEN total_count ELSE NULL END)::BIGINT > 0
$$;
