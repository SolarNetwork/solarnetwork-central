...
$$
	...
	SELECT m.node_id, m.source_id, ts::timestamp AT TIME ZONE nlt.time_zone
	FROM missing m
 ...
 * into the `solaragg.aud_datum_daily_stale` table so a record of the found node sources gets
 * generated.
 *
 * @param ts the date to look for; defaults to the current date
