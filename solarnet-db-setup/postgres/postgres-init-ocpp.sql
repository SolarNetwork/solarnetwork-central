...
CREATE TABLE solarev.ocpp_charge_point_conn (
	...
	CONSTRAINT ocpp_charge_point_conn_pk PRIMARY KEY (cp_id, evse_id, conn_id),
...
CREATE TABLE solarev.ocpp_charge_sess (
	...
	evse_id				INTEGER NOT NULL DEFAULT 0,
	conn_id				INTEGER NOT NULL,
	tx_id				VARCHAR(36) NOT NULL DEFAULT nextval('solarev.ocpp_charge_tx_seq'),
...
CREATE INDEX ocpp_charge_sess_conn_idx ON solarev.ocpp_charge_sess (cp_id, evse_id, conn_id);
 ...
 * user_id 			the account owner
 * cp_id			the charge point ID
 * evse_id			the EVSE ID (>= 0) related to the action, or 0 for the charger
...
CREATE TABLE solarev.ocpp_charge_point_action_status (
	...
	CONSTRAINT ocpp_charge_point_action_status_pk PRIMARY KEY (user_id, cp_id, evse_id, conn_id, action),
