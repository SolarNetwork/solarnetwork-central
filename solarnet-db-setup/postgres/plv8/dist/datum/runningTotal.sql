...
function runningTotal(sourceId) {
  ...
  function addInstantaneousValues(inst, weight) {
    ...
    for (prop in inst) {
      ...
      if (statMatch && inst[statMatch[1]] !== undefined) {
        ...
        (0, _addTo.default)(prop, inst[prop], iobj, weight);
      }
    }
  }
  ...
  function addDatumRecord(record) {
    ...
    var accu = record.jdata.a,
        inst = record.jdata.i,
        stat = record.jdata.s,
        tags = record.jdata.t; // add instantaneous values
  ...
  function finish() {
    ...
    for (prop in iobj) {
      ...
      aggInst[prop] = iobj[prop] / totalWeight; // inject min/max statistic values for instantaneous average values
    ...
    for (prop in aggInst) {
      ...
      aggRecord.jdata.i = (0, _mergeObjects.default)({}, aggInst);
      break;
    } // use for:in as easy test for an enumerable prop
