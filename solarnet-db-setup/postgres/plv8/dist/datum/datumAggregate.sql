...
var _auxiliaryHelper = _interopRequireDefault(require("datum/auxiliaryHelper"));

var _calculateAverages = _interopRequireDefault(require("../math/calculateAverages"));

var _calculateAverageOverHours = _interopRequireDefault(require("../math/calculateAverageOverHours"));
...
var kDefaultHourFill = {
  watts: 'wattHours'
};
var kAuxHelper = (0, _auxiliaryHelper.default)();
/**
...
function datumAggregate(sourceId, ts, endTs, configuration) {
  ...
  function addInstantaneousValues(inst) {
    ...
    for (prop in inst) {
      (0, _addTo.default)(prop, inst[prop], iobj, 1, iobjCounts, iobjStats);
    }
  }
  ...
  function addAccumulatingValues(accu, inst, recTs, recDate) {
    ...
    if (prevAccu) {
      for (prop in accu) {
        (0, _addTo.default)(prop, calculateAccumulatingValue(accu[prop], prevAccu[prop]), aobj, percent);
      }
    ...
    if (inst && prevInst) {
      for (prop in hourFill) {
        ...
        if ((!accu || accu[hourFill[prop]] === undefined) && inst[prop] !== undefined && prevInst[prop] !== undefined) {
          (0, _addTo.default)(hourFill[prop], (0, _calculateAverageOverHours.default)(inst[prop], prevInst[prop], recTimeDiff), aobj, percent);
        }
      }
    }
  }
  ...
  function addDatumRecord(record) {
    ...
    var recTs = record.ts_start.getTime(),
        ...
        stat = record.jdata.s,
        tags = record.jdata.t,
        resetType = kAuxHelper.datumResetKind(record.jdata); // as long as this record's time slot falls within the configured time slot,
    ...
    addAccumulatingValues(accu, inst, recTs, record.ts); // save curr record as previous for future calculations, unless we have a `start` reset type
  ...
  function finish(nextRecord) {
    var aggRecord = {
      ...
      jmeta: null
        ...
        aggInst,
        aggInstMeta,
        aggInstStat; // handle any fractional portion of the next record
    ...
    aggInst = (0, _calculateAverages.default)(iobj, iobjCounts); // store metadata about instantaneous calculations so further aggregation can be performed later

    aggInstMeta = {}; // inject min/max statistic values for instantaneous average values
    ...
    for (prop in aggInst) {
      ...
      aggInstMeta[prop] = {
        count: iobjCounts[prop] || 0
      };

      if (iobjStats[prop] !== undefined) {
        aggInstStat = (0, _fixPrecision.default)(iobjStats[prop].min);
        ...
        aggInstStat = (0, _fixPrecision.default)(iobjStats[prop].max);
