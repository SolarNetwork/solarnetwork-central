...
var _searchFilter = _interopRequireDefault(require("util/searchFilter"));
...
function walkObjectPathValues(obj, pathTokens, callback) {
  ...
  for (i = 0, end = pathTokens.length - 1; i <= end; i += 1) {
    ...
    if (pathToken === '*' && i === end) {
      ...
      for (prop in obj) {
        currPath[currPathIdx] = prop;
        val = obj[prop]; // check if prop after ** exists

        if (prop === pathTokens[i + 1] && val !== undefined && (_typeof(val) !== 'object' && i + 1 === end || _typeof(val) === 'object' && i + 1 < end)) {
          if (_typeof(val) !== 'object' && i + 1 === end) {
...
function objectPathMatcher(obj) {
  ...
  function matches(filterText) {
    return matchesFilter((0, _searchFilter.default)(filterText));
  }
  /**
   * Test if a filter matches the configured object.
   *
