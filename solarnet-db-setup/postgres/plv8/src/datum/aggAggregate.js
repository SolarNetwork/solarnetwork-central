 ...
 */
export default function aggAggregate(sourceId, ts, state) {
	var self = {
		version : '1'
	};
	...
	function addInstantaneousValues(inst, meta) {
		...
		for ( prop in inst ) {
			...
			if ( !propStats ) {
				imeta[prop] = {count:count, min:min, max:max};
			} else {
	...
	function calculateAggAverages(values, counts, meta) {
		var result = {}, prop, countSum, valArray, countArray, propMeta, i, len, avg;
		for ( prop in values ) {
